...
class MyLogisticRegression():
    ...
    def fit_(self, x, y):
        ...
        """
        if self.penalty == 'l2':
            self.fit_with_l2(x, y)
        else:
            self.fit_without_penalty(x, y)
    
    def fit_without_penalty(self, x, y):
        i = 0
    ...
    def fit_with_l2(self, x, y):
        ...
        m, _ = x.shape
        while i < self.max_iter:
            theta_ = np.array(self.theta)
            theta_[0] = 0
            X = np.insert(x, 0, np.array([1]), axis=1)
            grad = (1 / m) * (np.dot(X.transpose(), (1 / (1 + np.exp(-np.dot(X, self.theta))) - y)) + (self.lambda_ * theta_))
            self.theta = self.theta - self.alpha * (grad + self.lambda_ * self.theta)
            i += 1
